...
void Renderer::render(SDL_Renderer* sdl_renderer, World const& world) const
{
    ...
    for (size_t x = 0; x < size.x; ++x)
    {
        ...
        {
            SDL_Rect tile_rect{.x = (int32_t)(x * TILE_SIZE) + this->offset.x,
    ...
    for (size_t line_x = 0; line_x < size.x; ++line_x)
    {
        ...
        auto line_rect =
            SDL_Rect{.x = (int32_t)pixel_x + this->offset.x - (int32_t)(LINE_WIDTH / 2),
                     .y = 0 + this->offset.y,
                     .w = LINE_WIDTH,
                     .h = column_size};
    ...
    if (path_size > 0)
    {
        draw_line(sdl_renderer,
                  static_cast<glm::u32vec2>(world.map.test_entity_index) * TILE_SIZE +
                      TILE_SIZE / 2,
                  static_cast<glm::u32vec2>(world.map.test_path[path_size - 1]) * TILE_SIZE +
                      TILE_SIZE / 2,
                  path_color);
        for (size_t i = 0; path_size > 0 && i < path_size - 1; i++)
        {
            draw_line(sdl_renderer,
                      ...
                      static_cast<glm::u32vec2>(world.map.test_path[i + 1]) * TILE_SIZE +
                          TILE_SIZE / 2,
                      path_color);
        }
    }
