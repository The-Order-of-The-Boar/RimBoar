...
public:
    ...
    Graph(const int32_t node_count): node_count{node_count}
    {
        this->connections.resize(this->node_count);
    }

    Graph(const glm::i32vec2 world_size): node_count{world_size.x * world_size.y}
    {
        this->connections.resize(this->node_count);
    }

    void add_node_connection(const int32_t origin_node_id, const int32_t target_node_id,
                             const int32_t cost)
    {
